...
class Quiz extends React.Component {
    ...
    componentDidMount(){
        ...
        var url1 = `http://localhost:8000/api/questions/${quiz_id}`;
        var url2 = `http://localhost:8000/api/getanswers/${joinquiz_id}`;
        Promise.all([
    ...
    handleSubmit = (chosenoption) => (e) => {
        ...
        console.log(answers)
        fetch('http://localhost:8000/api/chosenanswer',{
            method: 'POST',
            headers: {
        ...
        .then(data => {
            fetch(`http://localhost:8000/api/getresults/${joinquiz_id}`)
            .then(response => response.json())
            .then(data => {
